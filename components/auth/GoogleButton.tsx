...
export function GoogleButton({ loading, setLoading, text = "Continue with Google" }: { loading: boolean, setLoading: (l: boolean) => void, text?: string }) {
    ...
    return (
        <Button type="button" className="w-full h-12 bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 font-medium rounded-xl shadow-sm transition-all" onClick={handleGoogleLogin} disabled={loading}>
